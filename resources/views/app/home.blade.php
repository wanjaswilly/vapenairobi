...
<meta name="description"
    content="The number one vape store in Nairobi. Contact us for quality vape disposables, vale E-liquids and vape devices. We offer paid delivery to all parts of the country.">
...
<div class="row px-5 gradient-header pb-5">
    <div class="col-lg-7 pt-5">
        ...
        <div class="long-desc">
            <span class="badge 
            @if ($introducing->productStatus == 'In Stock')
                badge-success
            @else
                badge-danger
            @endif 
            rounded-pill d-inline">{{$introducing->productStatus}}</span>
        </div>
        <div class="long-desc mb-3 mt-2">
    ...
    <div class="col-lg-5 pt-5">
        <a href="{{route('products.show', $introducing->id)}}" class="text-decoration-none text-dark">
            <img src="{{url('storage/' . $introducing->productCategory . '/' . $introducing->productBaseImage)}}"
                height="350px" class="card-img-top px-3 py-3 rounded hover-zoom" style="zoom:1.5;"
                alt="{{ $introducing->productName}}" />
        </a>
    </div>
</div>
                ...
                <a href="{{route('products.show', $product->id)}}" class="text-decoration-none text-dark">
                    <div class="mb-2">
                        <img src="{{url('storage/' . $product->productCategory . '/' . $product->productBaseImage)}}"
                            class="card-img-top px-3 py-3 rounded hover-zoom" height="450px" style="zoom:1.3;"
                            alt="{{$product->productName}}" />

                        <div class="text-center">
                            <h3><b>{{ $product->productName }}</b></h3>
                            <div class="long-desc mb-2">
                                <span class="badge 
                                    ...
                                    @else
                                        badge-danger
                                    @endif 
                                    rounded-pill d-inline">{{$product->productStatus}}</span>
                            </div>
        ...
        @forelse($eliquids as $product)
            <div class="col">
                <a href="{{route('products.show', $product->id)}}" class="text-decoration-none text-dark">
                    <div class=" mb-3 pb-2">
                        <img src="{{url('storage/' . $product->productCategory . '/' . $product->productBaseImage)}}"
                            class="card-img-top px-3 py-3 rounded hover-zoom" style="zoom:1.3;" height="350px"
                            alt="{{$product->productName}}" />
        ...
        @forelse($devices as $product)
            <div class="col">
                <a href="{{route('products.show', $product->id)}}" class="text-decoration-none text-dark">
                    <div class="">
                        <img src="{{url('storage/' . $product->productCategory . '/' . $product->productBaseImage)}}"
                            class="card-img-top px-3 py-3 rounded hover-zoom" height="350px" style="zoom:1.3;"
                            alt="{{$product->productName}}" />
